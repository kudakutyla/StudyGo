    ...
    <main className="flex min-h-screen items-center justify-center bg-[#f8f3ee] px-4 py-12">
      <div className="w-full max-w-md rounded-[2rem] border border-[#eadcca] bg-white p-8 shadow-soft">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-3xl font-semibold text-[#1f1a17]">Create your account</h1>
          <p className="mt-2 text-sm text-[#65554d]">Join StudyGo and stay on top of every assignment.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <div>
            <label htmlFor="name" className="mb-2 block text-sm font-medium text-[#2d241f]">Full Name</label>
            <input
              id="name"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              required
              className="w-full rounded-xl border border-[#d9cab8] bg-[#f9f5f2] px-3 py-2.5 text-[#1f1a17] outline-none transition focus:border-[#b7835a]"
          ...
          <div>
            <label htmlFor="confirmPassword" className="mb-2 block text-sm font-medium text-[#2d241f]">Confirm Password</label>
            <input
              id="confirmPassword"
              type={showPassword ? "text" : "password"}
        ...
        <p className="mt-5 text-center text-sm text-[#65554d]">
          Already have an account? <Link href="/login" className="font-medium text-[#3f312b]">Log in</Link>
        </p>
      </div>
    </main>
  );
